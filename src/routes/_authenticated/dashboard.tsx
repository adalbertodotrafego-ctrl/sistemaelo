  ...
  const { data: stats } = useQuery({
    ...
    queryFn: async () => {
      ...
      const [clientsActive, projectsActive, projectsDone, tasksPending,
        ...
        supabase.from("crm_leads").select("*", { count: "exact", head: true }).not("stage","in","(won,lost)"),
        supabase.from("campaigns").select("*", { count: "exact", head: true }),
        supabase.from("profiles").select("*", { count: "exact", head: true }).eq("status", "active"),
        supabase.from("finance_entries").select("amount").eq("kind","income").gte("paid_at", monthStart.toISOString().slice(0,10)),
      ]);
    ...
    <div className="space-y-8">
      <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}>
        <PageHeader
          ...
          description="Um panorama em tempo real da operação da Elo Marketing."
        />
      </motion.div>

      <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
        <StatCard label="Clientes ativos" value={stats?.clientsActive ?? "—"} icon={Users} accent="primary" />
        <StatCard label="Projetos ativos" value={stats?.projectsActive ?? "—"} icon={FolderKanban} accent="primary" />
        <StatCard label="Projetos concluídos" value={stats?.projectsDone ?? "—"} icon={Sparkles} accent="success" />
        <StatCard label="Receita do mês" value={brl(stats?.income ?? 0)} icon={Wallet} accent="success" />
        <StatCard label="Tarefas pendentes" value={stats?.tasksPending ?? "—"} icon={ListChecks} accent="warning" />
        <StatCard label="Reuniões hoje" value={stats?.meetingsToday ?? "—"} icon={Video} accent="primary" />
        <StatCard label="Leads em andamento" value={stats?.leads ?? "—"} icon={ArrowUpRight} accent="primary" />
        <StatCard label="Campanhas" value={stats?.campaigns ?? "—"} icon={Megaphone} accent="primary" />
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <div className="surface-card p-6">
          <div className="mb-4 flex items-center justify-between">
            <div className="font-display text-lg font-semibold">Tarefas em aberto</div>
